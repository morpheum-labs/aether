...
- [x] **Shared ABI spec** — `docs/agentscript-guest-abi.md` + `aether_common::guest_abi` constants (`VERSION`, reserved export names). When a job supplies WASM bytes, **`TeeRunner`** invokes **`aether_strategy_init`** + per-bar **`aether_strategy_step`** via **`GuestReplay::Ohlcv`**, so **`aether::series_*`** / **`series_hist*`** and host-side **`ta_*`** imports see the job dataset replay (`BarFeedState` in `aether-mwvm`). Reported **metrics / trades** are still a **placeholder** (flat equity) until guest state is surfaced. Without WASM, `VectorBacktestEngine` still drives results.
